...
def test_llm_executor(request: Request):
    ...
    uid = 700379634012798976
    thread_pool_executor.submit(test_run_llm, base_url, sql_engine, uid, access_token)
    return {'status': 'ok'}

# other thread
def test_run_llm(base_url, sql_engine, uid, access_token):
    logger.warning('test run begin')
    json_str = ''
    with SyncHttpClient(
        base_url=base_url,
    ...
    if res:
        with Session(sql_engine) as session:
            session.exec(
                update(User).where(User.uid == uid).values(tag=res))
            session.commit()
    logger.warning('test run finished')
...
@timeit
def handle_llm(client, messages):
    try:
        response = client.chat.completions.create(
            model="kimi-k2.6",
            messages=messages,
